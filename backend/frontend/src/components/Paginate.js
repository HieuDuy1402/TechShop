  ...
  return (
    pages > 1 && (
      <Pagination>
        ...
        {/* Prev */}
        {page > 1 ? (
          <Pagination.Prev href={`${url}page=${page - 1}`}>
            &laquo;
          </Pagination.Prev>
        ) : (
        ...
        {[...Array(pages).keys()].map((x) =>
          page === x + 1 ? (
            <Pagination.Item key={x + 1} active href={`${url}page=${x + 1}`}>
              {x + 1}
            </Pagination.Item>
          ) : x + 1 > page && x + 1 <= page + maxPageDisplay ? (
            <React.Fragment key={x + 1}>
              <Pagination.Item href={`${url}page=${x + 1}`}>
                {x + 1}
              </Pagination.Item>
              {x + 1 === page + maxPageDisplay && x + 1 < pages && (
                <Pagination.Ellipsis
                  href={`${url}page=${page + (maxPageDisplay + 1)}`}
                />
              )}
            </React.Fragment>
          ) : (
            x + 1 < page &&
            x + 1 >= page - maxPageDisplay && (
              <React.Fragment key={x + 1}>
        ...
        {/* Next */}
        {page < pages ? (
          <Pagination.Next href={`${url}page=${page + 1}`}>
            &raquo;
          </Pagination.Next>
        ) : (
